      ...
      <div className="space-y-2 rounded-xl border border-gray-200 bg-white shadow-sm p-4">
        ...
        {newUrl.trim() && (
          <button
            ...
            disabled={fetching}
            className="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 disabled:opacity-50"
          >
            {fetching ? "Fetching…" : "Fetch details from URL"}
          </button>
        )}
        {fetchNotice && <p className="text-xs text-gray-500">{fetchNotice}</p>}
        <input
          type="text"
      ...
      <div className="p-3">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1 space-y-1">
            <input
              ...
              className="w-full rounded-md border border-gray-200 px-2 py-1.5 text-sm font-semibold text-gray-900 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
            />
            <input
              type="text"
              value={jobTitle}
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          <label className="block">
            <span className="mb-1 block text-xs text-gray-500">Applied</span>
            <input
              type="date"
              value={appliedAt}
              onChange={(e) => setAppliedAt(e.target.value)}
              onBlur={saveAppliedAt}
              className="w-full rounded-md border border-gray-200 px-2 py-1.5 text-xs text-gray-600 focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary"
            />
          </label>
          <label className="block">
